...
class Network:
    ...
    def backprop(self, mini_batch):
        """
        :param mini_batch: is the mini batch of training examples in a
        list of tuples (x, y)
        ...
        """
        x = np.array([x.transpose()[0] for x, y in mini_batch]).transpose()
        y = np.array([y.transpose()[0] for x, y in mini_batch]).transpose()
        activation = x
        activations = [x]
        zs = []
        for b, w in zip(self.biases, self.weights):
        ...
        # print(delta)
        nabla_b[-1] = np.array([np.sum(delta, axis=1)]).transpose()
        # print(nabla_b[-1])
        # print(delta.shape)
        ...
        for l in range(2, self.num_layers):
            ...
            # print(delta.shape)
            # print(sp.shape)
            # print(np.dot(self.weights[-l+1].transpose(), delta).shape)
            delta = np.dot(self.weights[-l+1].transpose(), delta) * sp
            # input()
            nabla_b[-l] = np.array([np.sum(delta, axis=1)]).transpose()
